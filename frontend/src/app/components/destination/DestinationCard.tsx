...
export type DestinationCardDest = {
  id: string;
  title: string;
  subtitle?: string;
  category: string;
...
interface DestinationCardProps {
  dest: DestinationCardDest;
  renderId: string;
  index?: number;
  onSelect: (dest: DestinationCardDest, cardRect: DOMRect, renderId: string) => void;
}

export function DestinationCard({ dest, renderId, index = 0, onSelect }: DestinationCardProps) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="flex flex-col gap-2 p-3 md:p-4 text-left min-h-0">
        <div className="flex items-center justify-between gap-2 min-w-0">
          <h3 className="text-base md:text-lg font-bold text-gray-900 truncate font-serif flex-shrink min-w-0">
            {dest.title}
